...
export const Calendar: React.FC<CalendarProps> = ({ content, shared, pagesContent }) => {
  const cal = pagesContent?.calendar;
  const [selectedStart, setSelectedStart] = useState<StartType>('Fall');

  const startData = {
    'Fall': {
      label: cal?.fallStart ?? 'Fall Start (September)',
      month: 'September',
    ...
    'Winter': {
      label: cal?.winterStart ?? 'Winter Start (January)',
      month: 'January',
    ...
    'Spring': {
      label: cal?.springStart ?? 'Spring Start (May)',
      month: 'May',
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="mb-8 bg-gradient-to-r from-pau-darkBlue to-pau-blue rounded-2xl p-6 shadow-xl">
          ...
          <div className="relative">
            ...
            >
              <option value="Fall">{cal?.fallStart ?? 'Fall Start (September)'}</option>
              <option value="Winter">{cal?.winterStart ?? 'Winter Start (January)'}</option>
              <option value="Spring">{cal?.springStart ?? 'Spring Start (May)'}</option>
            </select>
        ...
        <div className="mb-12 bg-white shadow-soft rounded-2xl border-2 border-pau-gold/20 p-8 animate-fade-in">
          ...
          <div className="space-y-4 text-gray-700">
            <div className="flex items-start gap-3">
              ...
              <p>{cal?.bullet1 ?? <><strong>Trimester Structure:</strong> Each trimester is a four (4) month term, totaling sixteen (16) weeks, with mid-terms and final exams.</>}</p>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-pau-gold mt-2"></div>
              <p>{cal?.bullet2 ?? <><strong>Start Options:</strong> Students may commence their studies in January, May, or September.</>}</p>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-pau-gold mt-2"></div>
              <p>{cal?.bullet3 ?? <><strong>Academic Year:</strong> The next academic year cannot begin before the anniversary date of the start of the previous year.</>}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow-soft rounded-2xl border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-pau-light to-white px-8 py-6 border-b border-gray-200">
            <h3 className="text-xl font-serif font-bold text-pau-darkBlue">
              {cal?.scheduleTitle ?? '4-Year Course Schedule'}
            </h3>
            <p className="text-sm text-gray-500 mt-1">{cal?.scheduleSubtitle ? `${currentStart.label}${cal.scheduleSubtitle}` : `Complete course sequence for ${currentStart.label}`}</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-pau-darkBlue text-white">
                <tr>
                  <th className="px-6 py-3 text-left font-bold">{cal?.yearCol ?? 'Year'}</th>
                  <th className="px-6 py-3 text-left font-bold">{cal?.trimesterCol ?? 'Trimester'}</th>
                  <th className="px-6 py-3 text-left font-bold">{cal?.coursesCol ?? 'Courses'}</th>
                </tr>
              </thead>
        ...
        <div className="mt-12 bg-white shadow-soft rounded-2xl border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-pau-light to-white px-8 py-6 border-b border-gray-200">
            <h3 className="text-xl font-serif font-bold text-pau-darkBlue">
              {cal?.academicBreaks ?? 'Academic Breaks'}
            </h3>
          </div>
          <div className="px-8 py-6">
            <p className="text-gray-700 leading-relaxed mb-4">
              {cal?.breaksIntro ?? 'Each trimester includes a one-week break:'}
            </p>
            <ul className="space-y-2 text-gray-700">
              <li className="flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-pau-gold"></span>
                <span>{cal?.springBreak ? cal.springBreak : <><strong>Spring Break</strong> in late March</>}</span>
              </li>
              <li className="flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-pau-gold"></span>
                <span>{cal?.independenceDay ? cal.independenceDay : <><strong>Independence Day</strong> in July</>}</span>
              </li>
              <li className="flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-pau-gold"></span>
                <span>{cal?.thanksgiving ? cal.thanksgiving : <><strong>Thanksgiving</strong> in November</>}</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="flex items-start">
            ...
            <div className="ml-4">
              <h4 className="text-sm font-bold text-pau-darkBlue uppercase tracking-wider mb-2">
                {cal?.importantNote ?? 'Important Note'}
              </h4>
              <p className="text-sm text-gray-700 leading-relaxed">
                {cal?.importantNoteText ?? <>Application deadline is <span className="font-bold text-pau-blue">45 days prior</span> to your intended start date. Contact the Registrar at{' '}
